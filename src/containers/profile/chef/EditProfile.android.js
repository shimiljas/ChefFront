...
class EditProfile extends Component {
  ...
  handlePress(i) {
    ...
    if(i===0) {
      let arr = context.state.maxNumberOfImages;
      if(arr>0) {
      ...
      if(context.state.maxNumberOfVideos>0) {
  ...
  renderImageList(item){
    if(item.type == "thumbnail"){
      return(
        <TouchableOpacity delayLongPress={500} onLongPress={(e)=>{ this.deleteItemFromDataSource(item) }} >
          <Image
            style={{
    ...
    ImagePicker.openPicker({
      ...
      context.props.userActions.uploadImages(newImages, function(data) {
        ...
        context.setState({
          ...
          maxNumberOfImages: images.length > context.state.maxNumberOfImages ? 0 : 3-finalWorkHistory.length
        });
      });
