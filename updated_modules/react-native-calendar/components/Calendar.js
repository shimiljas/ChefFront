...
export default class Calendar extends Component {
  ...
  static propTypes = {
    ...
    bookingFullDates: PropTypes.array, // accepting array of booking full days
  ...
  prepareBookingFullDates(bookingFullDates) {
    const parsedDates = {};

    if(bookingFullDates) {
      bookingFullDates.forEach(event => {
        parsedDates[event] = {};
      });
    }
  ...
  renderCalendarView(calFormat, argMoment, eventsMap,bookingFullMap) {
    let renderIndex = 0,
        weekRows = [],
        days = [];
      ...
      if (dayIndex >= 0 && dayIndex < argDaysCount) {
        days.push((
          <Day
            ...
            bookingFullDates={bookingFullMap[thisMoment.format('YYYY-MM-DD')] ||
                          bookingFullMap[thisMoment.format('YYYYMMDD')]}
      ...
      ? (
        <View style={[styles.calendarControls, this.props.customStyle.calendarControls]}>
          <TouchableOpacity
             ...
             onPress={this.onPrev}
             >
            <Image source={Constants.Images.user.prev} style={{
              height:Constants.BaseStyle.DEVICE_WIDTH/100*4.6,
              width:Constants.BaseStyle.DEVICE_WIDTH/100*3.2,
          ...
          <TouchableOpacity style={styles.title} onPress={() => this.props.onTitlePress && this.props.onTitlePress()}>
            <Text style={[styles.titleText, this.props.customStyle.title]}>
              {localizedMonth}{moment(this.state.currentMoment.year(),"YYYY").format("YY")}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
             style={[styles.controlButton, this.props.customStyle.controlButton]}
             onPress={this.onNext}
             >
            <Image source={Constants.Images.user.next} style={{
              height:Constants.BaseStyle.DEVICE_WIDTH/100*4.6,
              width:Constants.BaseStyle.DEVICE_WIDTH/100*3.2,
              margin:Constants.BaseStyle.DEVICE_WIDTH/100*3,
            }} />
  ...
  render() {
    ...
    const prepareBookingFullMap  = this.prepareBookingFullDates(this.props.bookingFullDates)
      ...
      <View style={[styles.calendarContainer, this.props.customStyle.calendarContainer]}>
        ...
        {this.props.scrollEnabled ?
          <ScrollView
             ...
             >
              {calendarDates.map((date) => this.renderCalendarView(this.props.calendarFormat, moment(date), eventDatesMap,prepareBookingFullMap))}
