...
class AxisStruct {
  ...
  static calcStepSize(range, targetSteps,accurateDecimal)
  {
   
    const tempStep = range / targetSteps
    if(accurateDecimal==true)
      return tempStep;
    else{
  ...
  static getTickValues(axis, tickCount, decimalPlaces,days,accurateDecimal) {
    const tickStep = AxisStruct.calcStepSize((axis.maxValue - axis.minValue),tickCount,accurateDecimal)

    const maxTick = axis.minValue + (tickCount * tickStep)
      let tickValues;
    if(accurateDecimal==true){
     tickValues = _.range(axis.minValue, maxTick, tickStep)  
           if(tickValues.indexOf(axis.maxValue) == -1){
           // tickValues.splice(tickValues.length-1, 1);
            tickValues.push(axis.maxValue)
   ...
   // let tickValues = _.range(0, 21, 6)
   // let tickValues = _.range(axis.minValue, maxTick, tickStep)

   //let tickValues = _.range(axis.minValue, axis.maxValue+1,(axis.maxValue)/tickCount)
 
   if(tickCount==12 || tickCount==6 ){

   tickValues = _.range(axis.minValue, axis.maxValue+2,(axis.maxValue)/tickCount)
   if(tickValues.indexOf(axis.maxValue) == -1){
   tickValues.splice(tickValues.length-1, 1);
   tickValues.push(axis.maxValue)
   ...
   // tickValues = _.range(axis.minValue, maxTick, tickStep)
    ...
    tickValues = _.range(axis.minValue, days,(days-2)/tickCount) 
     if(tickValues.indexOf(days) == -1){
       tickValues.splice(tickValues.length-1, 1);
      tickValues.push(days)
  ...
  axis() {
    ...
    const tickInterval = this.options.tickCount || this.options.dataSlot || 4
    const accurateDecimal= this.options.accurateDecimal?true:false
    const decimalPlaces = this.options.decimalPlaces || 0
    const ticks = this.options.tickValues !== undefined && this.options.tickValues.length !== 0? _.map(this.options.tickValues,function(v){return v.value }):AxisStruct.getTickValues(currentAxis, tickInterval, decimalPlaces,this.options.days,accurateDecimal)
    ...
    const tailLength = this.options.tailLength || 1
...
export default class Axis extends Component {
  ...
  render() {
    ...
    const { chartArea, options, scale , datalen} = this.props

  //   console.log("this.options.ti",datalen)
      ...
      if (label !== undefined && label !== null) {
        returnValue =
          <G key={i} x={gxy[0]} y={gxy[1]}>
              ...
              {options.enableData == false && options.showLabels &&
                <Text x={xy[0]} y={xy[1]}
                ...
                {options.enableData == true && options.showLabels &&
                <Text 
                  textAnchor={
                    options && options.dataX !== undefined && label && options.dataX[label-1] 
                    && options.dataX[label-1].lifetime!== undefined ?"end":"middle"
                  } 
                  fill={textStyle.fill}
                  fontFamily={textStyle.fontFamily} 
                  fontSize={textStyle.fontSize}  
                  ...
                  dy={3}
                  >
                    {options && options.dataX !== undefined && options.dataX[label-1] 
                      && options.dataX[label-1].name !== undefined ? options.dataX[label-1].name : null}
                </Text>}
          </G>
      }
